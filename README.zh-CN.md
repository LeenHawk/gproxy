...
- **感觉不到网关的存在。** Rust 编写，零拷贝数据面。每个请求只增加约 0.2 ms，
  单机每秒结算 2.7 万个计费请求，2 万并发连接一个不丢。见[性能](#性能)。
...
GPROXY 的设计目标是网关永远不成为瓶颈。数据面以引用计数的字节块传递请求体，
流式响应不需要改写就原样透传；计量在响应离开之后进行，经过一个有上界的积压队列，
由 SQLite 线程把多笔结算合并成一次提交。没有一个请求逃过计费，也没有一个请求等待磁盘。

在一台笔记本级别的机器上（AMD Ryzen 7 8745H，16 线程，NVMe 上的 SQLite）
用本机 mock 上游实测，每个请求都完整走过鉴权、路由、计价并写入用量账本：

| 场景 | 结果 |
|---|---|
| 单连接附加延迟 | 中位数 0.21 ms |
| 计费吞吐，32 连接 | 27,000 请求/秒，p99 4.4 ms |
| 计费吞吐，2,000 连接 | 23,000 请求/秒 |
| 仅鉴权与路由 | 108,000 请求/秒 |
| 10,000 并发连接，上游 500 ms | 18,600 请求/秒，中位数只比上游本身多 1.4 ms |
| 20,000 并发连接，上游 500 ms | 18,500 请求/秒，零错误 |
| 10,000 个连接在线时的内存 | 约 1.2 GB |

压测客户端、网关和 mock 上游共用同一颗 16 核 CPU，独占一台机器只会更好。
接上真实模型后，请求在 GPROXY 里花的时间，和模型思考的时间相比只是个零头。
