...
/// Restore the public multipart wire shape after ingress normalized an audio
/// upload to JSON/data URLs for routing and transforms.
pub fn restore_audio_multipart(
    operation: Operation,
    ...
    let fields: Map<String, Value> = serde_json::from_slice(&body)
        .map_err(|error| ChannelError::Build(format!("audio upload body is not JSON: {error}")))?;
    let digest = blake3::hash(&body).to_hex();
    let boundary = format!("gproxy-audio-{}", &digest.as_str()[..24]);
    let mut output = Vec::with_capacity(body.len());
    for (name, value) in fields {
        append_value(&mut output, &boundary, &name, value)?;
    }
...
fn append_value(
    ...
    if let Value::Array(values) = value {
        ...
        for value in values {
            append_value(output, boundary, &array_name, value)?;
        }
        return Ok(());
    }
    if name == "file" {
        let data_url = value
            .as_str()
            .ok_or_else(|| ChannelError::Build("audio file must be a data URL".into()))?;
...
fn decode_data_url(value: &str) -> Result<(&str, Vec<u8>), ChannelError> {
    ...
    let mime = metadata
        .strip_suffix(";base64")
        .ok_or_else(|| ChannelError::Build("audio file data URL must be base64".into()))?;
    let data = base64::engine::general_purpose::STANDARD
        .decode(payload)
        .map_err(|error| ChannelError::Build(format!("invalid audio file base64: {error}")))?;
    Ok((mime, data))
}
...
mod tests {
    ...
    fn restores_binary_file_and_repeated_fields() {
        ...
        let output =
            restore_audio_multipart(Operation::CreateTranscription, &mut headers, body).unwrap();
