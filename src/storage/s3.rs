use anyhow::{Result, anyhow};
use arc_swap::ArcSwap;
use async_trait::async_trait;
use s3::creds::Credentials;
use s3::{Bucket, Region};
use std::sync::Arc;
use tokio::sync::{Mutex, mpsc, oneshot};
use tokio::time::{self, Duration, Instant};
use toml_edit::DocumentMut;
use url::Url;

use crate::config::AppConfig;
use crate::providers::ProvidersConfig;
use crate::storage::{ConfigStore, StorageConfig, StorageSettings};

pub struct S3Storage {
    bucket: Box<Bucket>,
    key: String,
    tx: mpsc::Sender<WriteRequest>,
    config: ArcSwap<AppConfig>,
    doc: Mutex<Option<DocumentMut>>,
    update_lock: Mutex<()>,
    debounce: Duration,
    storage_config: StorageConfig,
}

impl S3Storage {
    pub fn connect(settings: &StorageSettings, storage_config: StorageConfig) -> Result<Self> {
        let StorageSettings::S3 {
            bucket,
            region,
            access_key,
            secret_key,
            endpoint,
            path_style,
            session_token,
            use_tls,
            path,
            debounce_secs,
        } = settings
        else {
            return Err(anyhow!("storage settings mismatch for s3"));
        };

        let credentials = Credentials::new(
            Some(access_key),
            Some(secret_key),
            None,
            session_token.as_deref(),
            None,
        )?;

        let region = match endpoint {
            Some(endpoint) => Region::Custom {
                region: region.clone(),
                endpoint: normalize_endpoint(endpoint, *use_tls)?,
            },
            None => region
                .parse()
                .map_err(|err| anyhow!("invalid s3 region: {}", err))?,
        };

        let mut bucket = Bucket::new(bucket, region, credentials)?;
        if path_style.unwrap_or(false) {
            bucket.set_path_style();
        }

        let key = normalize_key(path);
        let (tx, rx) = mpsc::channel(32);
        let writer_bucket = bucket.clone();
        let writer_key = key.clone();
        let debounce = Duration::from_secs(*debounce_secs);
        tokio::spawn(async move {
            write_worker(writer_bucket, writer_key, rx, debounce).await;
        });

        Ok(Self {
            bucket,
            key,
            tx,
            config: ArcSwap::from_pointee(AppConfig::default()),
            doc: Mutex::new(None),
            update_lock: Mutex::new(()),
            debounce,
            storage_config,
        })
    }

    pub fn debounce(&self) -> Duration {
        self.debounce
    }

    pub async fn providers_get<R, F>(&self, read: F) -> Result<R>
    where
        F: FnOnce(&ProvidersConfig) -> R + Send,
    {
        let config = self.config.load_full();
        Ok(read(&config.providers))
    }

    pub async fn providers_update<F>(&self, update: F) -> Result<()>
    where
        F: FnOnce(&mut ProvidersConfig, Option<&mut DocumentMut>) -> Result<()> + Send,
    {
        let mut config = (*self.config.load_full()).clone();
        self.with_document_mut(|doc| update(&mut config.providers, Some(doc)))
            .await?;
        self.config.store(Arc::new(config));
        Ok(())
    }

    pub(crate) async fn lock_update(&self) -> tokio::sync::MutexGuard<'_, ()> {
        self.update_lock.lock().await
    }

    pub(crate) async fn object_exists(&self) -> Result<bool> {
        self.bucket
            .object_exists(self.key.as_str())
            .await
            .map_err(|err| err.into())
    }

    pub(crate) fn update_cached_config<F>(&self, update: F) -> AppConfig
    where
        F: FnOnce(&mut AppConfig),
    {
        let mut next = (*self.config.load_full()).clone();
        update(&mut next);
        self.config.store(Arc::new(next.clone()));
        next
    }

    pub(crate) async fn with_document_mut<F, R>(&self, apply: F) -> Result<R>
    where
        F: FnOnce(&mut DocumentMut) -> Result<R>,
    {
        let mut doc_guard = self.doc.lock().await;
        if doc_guard.is_none() {
            let config = self.config.load_full();
            let contents = toml::to_string(config.as_ref())?;
            *doc_guard = Some(contents.parse()?);
        }
        let doc = doc_guard.as_mut().expect("document cached");
        let result = apply(doc)?;
        let contents = doc.to_string();
        self.tx
            .send(WriteRequest::WriteRaw(contents))
            .await
            .map_err(|_| anyhow!("storage writer task closed"))?;
        Ok(result)
    }

    async fn write_raw_now(&self, contents: &str) -> Result<()> {
        write_raw(&self.bucket, self.key.as_str(), contents.as_bytes()).await
    }
}

#[async_trait]
impl ConfigStore for S3Storage {
    async fn get_app_config(&self) -> Result<AppConfig> {
        Ok((*self.config.load_full()).clone())
    }

    async fn load_app_config(&self) -> Result<AppConfig> {
        let exists = self.bucket.object_exists(self.key.as_str()).await?;
        let (config, contents) = if !exists {
            let config = AppConfig {
                storage: self.storage_config.clone(),
                ..Default::default()
            };
            let contents = toml::to_string(&config)?;
            self.write_raw_now(&contents).await?;
            (config, contents)
        } else {
            let response = self.bucket.get_object(self.key.as_str()).await?;
            let contents = response
                .to_string()
                .map_err(|err| anyhow!("s3 config is not valid utf-8: {}", err))?;
            let config = toml::from_str(&contents)?;
            (config, contents)
        };
        self.config.store(Arc::new(config.clone()));
        let mut doc_guard = self.doc.lock().await;
        *doc_guard = Some(contents.parse()?);
        Ok(config)
    }

    async fn save_app_config(&self, config: &AppConfig) -> Result<()> {
        let contents = toml::to_string(config)?;
        let mut doc_guard = self.doc.lock().await;
        *doc_guard = Some(contents.parse()?);
        self.config.store(Arc::new(config.clone()));
        self.tx
            .send(WriteRequest::Write(Box::new(config.clone())))
            .await
            .map_err(|_| anyhow!("storage writer task closed"))?;
        Ok(())
    }

    async fn flush_app_config(&self) -> Result<()> {
        let (tx, rx) = oneshot::channel();
        self.tx
            .send(WriteRequest::Flush(tx))
            .await
            .map_err(|_| anyhow!("storage writer task closed"))?;
        rx.await
            .map_err(|_| anyhow!("storage writer task closed"))?
    }
}

enum WriteRequest {
    Write(Box<AppConfig>),
    WriteRaw(String),
    Flush(oneshot::Sender<Result<()>>),
}

async fn write_worker(
    bucket: Box<Bucket>,
    key: String,
    mut rx: mpsc::Receiver<WriteRequest>,
    debounce: Duration,
) {
    while let Some(request) = rx.recv().await {
        let mut flush_waiters: Vec<oneshot::Sender<Result<()>>> = Vec::new();
        let mut pending = match request {
            WriteRequest::Write(config) => Some(WritePayload::Config(config)),
            WriteRequest::WriteRaw(raw) => Some(WritePayload::Raw(raw)),
            WriteRequest::Flush(tx) => {
                let _ = tx.send(Ok(()));
                continue;
            }
        };

        if debounce.is_zero() {
            loop {
                match rx.try_recv() {
                    Ok(WriteRequest::Write(config)) => {
                        pending = Some(WritePayload::Config(config));
                    }
                    Ok(WriteRequest::WriteRaw(raw)) => {
                        pending = Some(WritePayload::Raw(raw));
                    }
                    Ok(WriteRequest::Flush(tx)) => {
                        flush_waiters.push(tx);
                        break;
                    }
                    Err(mpsc::error::TryRecvError::Empty) => break,
                    Err(mpsc::error::TryRecvError::Disconnected) => break,
                }
            }
        } else {
            let deadline = Instant::now() + debounce;
            loop {
                let sleep = time::sleep_until(deadline);
                tokio::pin!(sleep);
                tokio::select! {
                    _ = &mut sleep => break,
                    message = rx.recv() => {
                        match message {
                            Some(WriteRequest::Write(config)) => {
                                pending = Some(WritePayload::Config(config));
                            }
                            Some(WriteRequest::WriteRaw(raw)) => {
                                pending = Some(WritePayload::Raw(raw));
                            }
                            Some(WriteRequest::Flush(tx)) => {
                                flush_waiters.push(tx);
                                break;
                            }
                            None => break,
                        }
                    }
                }
            }
        }

        if let Some(payload) = pending {
            let result = match payload {
                WritePayload::Config(config) => {
                    write_config(&bucket, key.as_str(), config.as_ref()).await
                }
                WritePayload::Raw(raw) => write_raw(&bucket, key.as_str(), raw.as_bytes()).await,
            };
            let err_msg = result.as_ref().err().map(|err| err.to_string());
            for waiter in flush_waiters {
                let send_result = match &err_msg {
                    Some(msg) => Err(anyhow!(msg.clone())),
                    None => Ok(()),
                };
                let _ = waiter.send(send_result);
            }
        }
    }
}

enum WritePayload {
    Config(Box<AppConfig>),
    Raw(String),
}

async fn write_config(bucket: &Bucket, key: &str, config: &AppConfig) -> Result<()> {
    let contents = toml::to_string(config)?;
    write_raw(bucket, key, contents.as_bytes()).await?;
    Ok(())
}

async fn write_raw(bucket: &Bucket, key: &str, contents: &[u8]) -> Result<()> {
    bucket.put_object(key, contents).await?;
    Ok(())
}

fn normalize_key(path: &str) -> String {
    let trimmed = path.trim();
    let trimmed = trimmed.trim_start_matches('/');
    if trimmed.is_empty() {
        StorageSettings::default_s3_path()
    } else {
        trimmed.to_string()
    }
}

fn normalize_endpoint(endpoint: &Url, use_tls: Option<bool>) -> Result<String> {
    let mut endpoint = endpoint.clone();
    if let Some(use_tls) = use_tls {
        let scheme = if use_tls { "https" } else { "http" };
        endpoint
            .set_scheme(scheme)
            .map_err(|_| anyhow!("invalid s3 endpoint scheme"))?;
    }
    Ok(endpoint.to_string())
}
