...
pub async fn open_remote_control_websocket(
    ...
    let provider = cp.providers_by_name.get(provider_name)
        .filter(|provider| provider.enabled && provider.channel == "codex")
        .cloned().ok_or_else(|| PipelineError::UnknownProvider(provider_name.clone()))?;
    let binding_key = remote_control_token_key(provider.id, &remote_token);
    drop(cp);
    let credential_id = state.cache.get(&binding_key).await
    ...
    crate::credentials::control::open_raw_websocket(state, credential_id, operation)
        .await.map_err(|error| PipelineError::Transport(error.to_string()))
}
...
async fn run(state: &AppState, ctx: &mut RequestCtx) -> Result<ExecOutcome, PipelineError> {
    ...
    match (ctx.method.as_str(), ctx.path.as_str()) {
        ...
        | ("POST", "/backend-api/codex/analytics-events/events") => {
            tracing::info!(user_id = identity.user.id, "accepted virtual Codex analytics events");
            return Ok(json_outcome(StatusCode::OK, json!({})));
        }
        _ => {}
    }
    ...
    let (label, upstream_path) = allowlisted_upstream(&ctx.method, &ctx.path)
...
async fn virtual_thread_usage(
    ...
    for thread_id in ids.iter().filter_map(Value::as_str) {
        ...
        let cost = rows.iter().fold(rust_decimal::Decimal::ZERO, |sum, row| sum + row.cost);
...
async fn local_task_list(
    ...
    let params = query
        ...
        .unwrap_or_default();
    let value = |name: &str| params.iter().find_map(|(key, value)| (key == name).then_some(value.as_str()));
    let limit = value("limit").and_then(|value| value.parse::<usize>().ok()).unwrap_or(20).clamp(1, 100);
    let environment = value("environment_id");
    let task_filter = value("task_filter");
    let cursor = value("cursor").and_then(decode_task_cursor);
    let mut rows = state.persistence
    ...
    let mut filtered = rows.into_iter().filter(|row| {
        environment.is_none_or(|environment| row.environment_id.as_deref() == Some(environment))
            ...
            && task_filter.is_none_or(|filter| match filter {
                "archived" => row.summary_json.get("archived").and_then(Value::as_bool) == Some(true),
                "current" => row.summary_json.get("archived").and_then(Value::as_bool) != Some(true),
                _ => true,
            })
    });
    let page = filtered.by_ref().take(limit + 1).collect::<Vec<_>>();
    let has_more = page.len() > limit;
    let items = page.iter().take(limit).map(|row| row.summary_json.clone()).collect::<Vec<_>>();
    let next = has_more.then(|| {
        let row = &page[limit - 1];
        encode_task_cursor(row.updated_at, row.id)
    });
    Ok(json_outcome(StatusCode::OK, json!({ "items": items, "cursor": next })))
}
...
async fn create_task(
    ...
    let requested_environment = request_value.pointer("/new_task/environment_id").and_then(Value::as_str);
    ...
    if response.disposition == Disposition::Success {
        ...
        let task_id = value.pointer("/task/id").or_else(|| value.get("id")).and_then(Value::as_str);
        if let Some(task_id) = task_id {
            let environment_id = request_value.pointer("/new_task/environment_id").and_then(Value::as_str).map(str::to_owned);
            let title = task_title(&request_value);
            let now = unix_now();
            state.persistence.upsert_codex_task_binding(CodexTaskBindingInput {
    ...
    Ok(ExecOutcome { status: response.status, headers: response.headers, body: ResponseBody::Full(response.body), disposition: response.disposition })
}
...
async fn aggregate_environments(
    ...
    if credentials.is_empty() { return Err(PipelineError::NoCredentials); }
    let rest = ctx.path.strip_prefix("/api/codex/").ok_or(PipelineError::UnsupportedPath)?;
    ...
    for credential in credentials {
        ...
        let Ok(response) = crate::credentials::control::execute_raw(state, credential.id, operation).await else { continue; };
        if response.disposition != Disposition::Success { continue; }
        let Ok(Value::Array(items)) = serde_json::from_slice::<Value>(&response.body) else { continue; };
        for item in items {
            let Some(id) = item.get("id").and_then(Value::as_str).map(str::to_owned) else { continue; };
    ...
    for (environment_id, credential_ids) in owners {
        let value = credential_ids.iter().map(i64::to_string).collect::<Vec<_>>().join(",");
        let _ = state.cache.set(&environment_key(provider.id, &environment_id), value.into_bytes(), Some(std::time::Duration::from_secs(900))).await;
    }
    Ok(json_outcome(
        StatusCode::OK,
...
fn environment_key(provider_id: i64, environment_id: &str) -> String {
    format!("codex_env:{provider_id}:{}", blake3::hash(environment_id.as_bytes()).to_hex())
}

async fn environment_credentials(state: &AppState, provider_id: i64, environment_id: &str) -> Option<Vec<i64>> {
    let value = state.cache.get(&environment_key(provider_id, environment_id)).await?;
    String::from_utf8(value).ok()?.split(',').map(str::parse).collect::<Result<Vec<_>, _>>().ok()
}
...
async fn bound_task_request(
    ...
    let binding = state.persistence.get_codex_task_binding(provider.id, task_id).await
        .map_err(|error| PipelineError::Transport(error.to_string()))?
    ...
    let response = crate::credentials::control::execute_raw(state, binding.credential_id, operation).await
        .map_err(|error| PipelineError::Transport(error.to_string()))?;
    if ctx.method == Method::GET && response.disposition == Disposition::Success {
        let value = serde_json::from_slice::<Value>(&response.body).unwrap_or(Value::Null);
        let mut summary = binding.summary_json.clone();
        if let Some(object) = summary.as_object_mut() {
            if let Some(status) = value.get("task_status_display").or_else(|| value.pointer("/task/task_status_display")) {
                object.insert("task_status_display".to_owned(), status.clone());
            }
            object.insert("updated_at".to_owned(), Value::from(unix_now()));
        }
        let _ = state.persistence.upsert_codex_task_binding(CodexTaskBindingInput {
...
fn task_title(request: &Value) -> String {
    request.get("input_items").and_then(Value::as_array).into_iter().flatten()
        .flat_map(|item| item.get("content").and_then(Value::as_array).into_iter().flatten())
        .filter_map(|part| part.get("text").and_then(Value::as_str))
        .next().unwrap_or("Codex task").chars().take(120).collect()
}
...
fn virtual_ids(identity: &crate::app::snapshot::KeyIdentity, provider: &Provider) -> (String, String) {
    (
...
fn whoami(identity: &crate::app::snapshot::KeyIdentity, provider: &Provider) -> Value {
    ...
    let email = identity.user.name.contains('@').then(|| identity.user.name.clone());
    json!({
        "email": email,
...
async fn virtual_profile(
    ...
    let summary = state.persistence.summarize_usages(&crate::store::persistence::UsageQuery {
        provider_id: Some(provider.id),
        user_id: Some(identity.user.id),
        ..Default::default()
    }).await.map_err(|error| PipelineError::Transport(error.to_string()))?;
    let lifetime = summary.input_tokens.saturating_add(summary.output_tokens);
    Ok(json_outcome(StatusCode::OK, json!({
        "stats": {
...
async fn virtual_usage(
    ...
    let reached = [primary.as_ref(), secondary.as_ref()]
        .into_iter()
        .flatten()
        .any(|window| window.get("used_percent").and_then(Value::as_i64).unwrap_or(0) >= 100);
    json!({
    ...
    quotas
        ...
        .filter_map(|quota| {
            let (limit, used, anchor) = if five_hour {
                (quota.quota_5h?, quota.five_hour_used, quota.five_hour_anchor)
            } else {
                (quota.quota_7d?, quota.seven_day_used, quota.seven_day_anchor)
            };
            ...
            let percent = if limit_f <= 0.0 { 100 } else { ((used_f / limit_f) * 100.0).clamp(0.0, 100.0).round() as i64 };
...
fn allowlisted_upstream(method: &Method, path: &str) -> Option<(&'static str, String)> {
    ...
    let allowed = match (method.as_str(), rest) {
        ...
        ("GET" | "POST", value) if value == "tasks" || value.starts_with("tasks/") || value == "tasks/list" => {
        ...
        ("GET" | "POST" | "DELETE", value) if value.starts_with("remote/control/") || value == "remote/control" => {
            ("remote_control", format!("/wham/{value}"))
        }
        ("POST", value) if matches!(value, "responses" | "responses/compact" | "images/generations" | "images/edits" | "alpha/search" | "memories/trace_summarize" | "realtime/calls") => {
...
fn remote_control_token_key(provider_id: i64, token: &str) -> String {
    format!("codex_rc_token:{provider_id}:{}", blake3::hash(token.as_bytes()).to_hex())
}
...
async fn execute_balanced(
    ...
    let mcp_session = match &operation {
        CredentialControlOperation::CodexRaw { label, headers, .. } if *label == "ps_mcp" => headers
            .get("mcp-session-id")
    ...
    let binding_key = mcp_session.as_deref().map(|session| {
        format!("codex_mcp:{}:{user_id}:{}", provider.id, blake3::hash(session.as_bytes()).to_hex())
    });
    let remote_server = match &operation {
        CredentialControlOperation::CodexRaw { label, body, headers, .. } if *label == "remote_control" => {
            serde_json::from_slice::<Value>(body).ok()
                .and_then(|value| value.get("server_id").and_then(Value::as_str).map(str::to_owned))
                .or_else(|| headers.get("x-codex-server-id").and_then(|value| value.to_str().ok()).map(str::to_owned))
        }
        _ => None,
    };
    let remote_binding_key = remote_server.as_deref().map(|server_id| {
        format!("codex_rc_server:{}:{user_id}:{}", provider.id, blake3::hash(server_id.as_bytes()).to_hex())
    });
    let plugin_id = match &operation {
        CredentialControlOperation::CodexRaw { label, path, .. } if *label == "plugins" => {
            path.strip_prefix("/ps/plugins/").and_then(|rest| rest.split('/').next())
                .filter(|id| !matches!(*id, "list" | "installed" | "search" | "suggested" | "workspace"))
                .map(str::to_owned)
        }
        _ => None,
    };
    let plugin_binding_key = plugin_id.as_deref().map(|plugin_id| {
        format!("codex_plugin:{}:{user_id}:{}", provider.id, blake3::hash(plugin_id.as_bytes()).to_hex())
    });
    let pinned = match binding_key.as_deref().or(remote_binding_key.as_deref()).or(plugin_binding_key.as_deref()) {
        Some(key) => state.cache.get(key).await.and_then(|value| String::from_utf8(value).ok()).and_then(|value| value.parse().ok()),
        None => None,
    };
    let credentials = {
        let cp = state.cp();
    ...
    if is_mcp || is_model_stream {
        let credential = credentials.first().ok_or(PipelineError::NoCredentials)?;
        let response = crate::credentials::control::execute_raw_streaming(
            state,
            credential.id,
            operation,
        )
    ...
    for credential in credentials {
        match crate::credentials::control::execute_raw(state, credential.id, operation.clone()).await {
            Ok(response) => {
                if response.disposition == Disposition::Success
                ...
                {
                    let _ = state.cache.set(key, credential.id.to_string().into_bytes(), Some(std::time::Duration::from_secs(30 * 86_400))).await;
                }
                if response.disposition == Disposition::Success
                    && matches!(&operation, CredentialControlOperation::CodexRaw { label, .. } if *label == "remote_control")
                    && let Ok(value) = serde_json::from_slice::<Value>(&response.body)
                ...
                {
                    let key = format!("codex_rc_server:{}:{user_id}:{}", provider.id, blake3::hash(server_id.as_bytes()).to_hex());
                    let _ = state.cache.set(&key, credential.id.to_string().into_bytes(), Some(std::time::Duration::from_secs(7 * 86_400))).await;
                }
                if response.disposition == Disposition::Success
                    && matches!(&operation, CredentialControlOperation::CodexRaw { label, .. } if *label == "ps_mcp")
                    && let Some(session) = response.headers.get("mcp-session-id").and_then(|value| value.to_str().ok())
                {
                    let key = format!("codex_mcp:{}:{user_id}:{}", provider.id, blake3::hash(session.as_bytes()).to_hex());
                    let _ = state.cache.set(&key, credential.id.to_string().into_bytes(), Some(std::time::Duration::from_secs(24 * 3600))).await;
                }
                if response.disposition == Disposition::Success
                    || !retryable
    ...
    Err(PipelineError::Transport(
        last_error.unwrap_or_else(|| "all Codex service credentials failed".to_owned()),
    ))
}
