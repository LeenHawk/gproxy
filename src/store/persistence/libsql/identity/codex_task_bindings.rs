...
pub async fn get(client: &LibsqlClient, provider_id: i64, task_id: &str) -> anyhow::Result<Option<CodexTaskBinding>> {
    query_one(client, &format!("SELECT {COLS} FROM codex_task_bindings WHERE provider_id = ? AND task_id = ?"), &[arg_integer(provider_id), arg_text(task_id)])
        .await?.as_ref().map(decode).transpose()
}

pub async fn list(client: &LibsqlClient, provider_id: i64, owner_user_id: i64) -> anyhow::Result<Vec<CodexTaskBinding>> {
...
pub async fn upsert(client: &LibsqlClient, input: CodexTaskBindingInput) -> anyhow::Result<CodexTaskBinding> {
    let now = now_secs();
    ...
    get(client, input.provider_id, &input.task_id).await?.ok_or_else(|| anyhow::anyhow!("codex task binding vanished after upsert"))
}

pub async fn delete_by_user(client: &LibsqlClient, owner_user_id: i64) -> anyhow::Result<()> {
    exec(client, "DELETE FROM codex_task_bindings WHERE owner_user_id = ?", &[arg_integer(owner_user_id)]).await?;
    Ok(())
}
