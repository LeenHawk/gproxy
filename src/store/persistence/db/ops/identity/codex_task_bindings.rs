...
pub async fn get(conn: &DatabaseConnection, provider_id: i64, task_id: &str) -> anyhow::Result<Option<CodexTaskBinding>> {
    codex_task_binding::Entity::find()
        ...
        .one(conn).await?.map(record).transpose()
}

pub async fn list(conn: &DatabaseConnection, provider_id: i64, owner_user_id: i64) -> anyhow::Result<Vec<CodexTaskBinding>> {
    codex_task_binding::Entity::find()
        .filter(codex_task_binding::Column::ProviderId.eq(provider_id))
        .filter(codex_task_binding::Column::OwnerUserId.eq(owner_user_id))
        .all(conn).await?.into_iter().map(record).collect()
}

pub async fn upsert(conn: &DatabaseConnection, input: CodexTaskBindingInput) -> anyhow::Result<CodexTaskBinding> {
    ...
    let model = match codex_task_binding::Entity::find()
        ...
        None => codex_task_binding::ActiveModel {
            id: NotSet,
