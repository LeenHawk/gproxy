...
pub const MIGRATIONS: &[Migration] = &[
    ...
    Migration {
        version: 8,
        description: "price_rules: provider/global exact/contains pricing resolver",
        sql: MigrationSql::ByDialect {
            sqlite: &["CREATE TABLE IF NOT EXISTS price_rules (\
                    id INTEGER PRIMARY KEY, \
                    provider_id INTEGER, \
                    ...
                    created_at INTEGER NOT NULL, \
                    updated_at INTEGER NOT NULL)"],
            postgres: &["CREATE TABLE IF NOT EXISTS price_rules (\
                    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, \
                    provider_id BIGINT, \
                    match_type TEXT NOT NULL, \
                    model_match TEXT NOT NULL, \
                    ...
                    created_at BIGINT NOT NULL, \
                    updated_at BIGINT NOT NULL)"],
            mysql: &["CREATE TABLE IF NOT EXISTS price_rules (\
                    id BIGINT NOT NULL AUTO_INCREMENT, \
                    provider_id BIGINT, \
                    match_type VARCHAR(32) NOT NULL, \
                    model_match VARCHAR(255) NOT NULL, \
                    input_price TEXT NOT NULL, \
                    output_price TEXT NOT NULL, \
                    cache_read_price TEXT NOT NULL, \
                    cache_creation_5m_price TEXT NOT NULL, \
    ...
    Migration {
        version: 12,
        description: "credential_model_statuses: health snapshot per final upstream model",
        sql: MigrationSql::ByDialect {
            sqlite: &["CREATE TABLE IF NOT EXISTS credential_model_statuses (\
                    id INTEGER PRIMARY KEY, \
            ...
            postgres: &["CREATE TABLE IF NOT EXISTS credential_model_statuses (\
                    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, \
                    ...
                    updated_at BIGINT NOT NULL, \
                    UNIQUE(credential_id, channel, model_id))"],
            mysql: &["CREATE TABLE IF NOT EXISTS credential_model_statuses (\
                    id BIGINT NOT NULL AUTO_INCREMENT, \
                    credential_id BIGINT NOT NULL, \
                    channel VARCHAR(255) NOT NULL, \
                    model_id VARCHAR(255) NOT NULL, \
                    health_kind VARCHAR(255) NOT NULL, \
                    health_json TEXT, \
                    checked_at BIGINT, \
                    last_error TEXT, \
                    created_at BIGINT NOT NULL, \
                    updated_at BIGINT NOT NULL, \
                    PRIMARY KEY (id), \
                    UNIQUE KEY uq_credential_model_statuses_dims \
