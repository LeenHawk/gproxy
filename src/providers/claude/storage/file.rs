...
#[async_trait]
impl ClaudeBackend for FileStorage {
        ...
        F: FnOnce(&mut ClaudeSetting) + Send,
    {
        ...
        let config = self.update_cached_config(|config| {
            ...
            updated = Some(config.providers.claude.setting.clone());
        });
        let setting = updated.unwrap_or(config.providers.claude.setting);
    ...
    async fn add_credential(&self, credential: ClaudeCredential) -> Result<()> {
        ...
        self.with_document_mut(|doc| {
            write_credentials(doc, &config.providers.claude.credentials);
            Ok(())
        })
        .await
    }
...
fn write_setting(doc: &mut DocumentMut, setting: &ClaudeSetting) {
    let setting_item = doc["providers"]["claude"]["setting"].or_insert(Item::Table(Table::new()));
...
fn write_credentials(doc: &mut DocumentMut, credentials: &[ClaudeCredential]) {
    ...
    doc["providers"]["claude"]["credentials"] = Item::ArrayOfTables(array);
}
