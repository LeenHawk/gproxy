...
#[async_trait]
impl CodexBackend for FileStorage {
        ...
        F: FnOnce(&mut CodexSetting) + Send,
    {
        ...
        let config = self.update_cached_config(|config| {
            ...
            updated = Some(config.providers.codex.setting.clone());
        });
        let setting = updated.unwrap_or(config.providers.codex.setting);
    ...
    async fn add_credential(&self, credential: CodexCredential) -> Result<()> {
        ...
        self.with_document_mut(|doc| {
            write_credentials(doc, &config.providers.codex.credentials);
            Ok(())
        })
        .await
    }
...
fn write_setting(doc: &mut DocumentMut, setting: &CodexSetting) {
    let setting_item = doc["providers"]["codex"]["setting"].or_insert(Item::Table(Table::new()));
...
fn write_credentials(doc: &mut DocumentMut, credentials: &[CodexCredential]) {
    let mut array = ArrayOfTables::new();
    for credential in credentials {
        let mut table = Table::new();
        table["id_token"] = Value::from(credential.id_token.clone()).into();
        ...
        table["account_id"] = Value::from(credential.account_id.clone()).into();
        table["last_refresh"] = Value::from(credential.last_refresh).into();
        if let Some(states) = states_to_item(&credential.states) {
            table["states"] = states;
        }
        array.push(table);
    }
    doc["providers"]["codex"]["credentials"] = Item::ArrayOfTables(array);
}
