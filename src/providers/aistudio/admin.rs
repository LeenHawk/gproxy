...
pub struct AIStudioAdmin;

#[async_trait]
impl ProviderAdmin for AIStudioAdmin {
    async fn get_config(&self, ctx: &AppContext) -> Result<Value, StatusCode> {
        let storage = ctx.aistudio();
        let config = storage
            .get_config()
            .await
    ...
    async fn add_credential(&self, ctx: &AppContext, credential: Value) -> Result<(), StatusCode> {
        let credential: AIStudioCredential =
        ...
        let _ = set_default_usage_anchors(ctx.usage_store().as_ref(), ProviderKind::AIStudio, now)
            .await;
        for spec in DEFAULT_USAGE_VIEWS {
            ...
            ctx.schedule_usage_anchor(ProviderKind::AIStudio, spec.name, next_until)
                .await;
        }
        Ok(())
    }
