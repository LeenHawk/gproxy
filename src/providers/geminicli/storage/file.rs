...
#[async_trait]
impl GeminiCliBackend for FileStorage {
        ...
        F: FnOnce(&mut GeminiCliSetting) + Send,
    {
        ...
        let config = self.update_cached_config(|config| {
            ...
            updated = Some(config.providers.geminicli.setting.clone());
        });
        let setting = updated.unwrap_or(config.providers.geminicli.setting);
    ...
    async fn add_credential(&self, credential: GeminiCliCredential) -> Result<()> {
        ...
        self.with_document_mut(|doc| {
            write_credentials(doc, &config.providers.geminicli.credentials);
            Ok(())
        })
        .await
    }
...
fn write_setting(doc: &mut DocumentMut, setting: &GeminiCliSetting) {
    let setting_item =
        doc["providers"]["geminicli"]["setting"].or_insert(Item::Table(Table::new()));
...
fn write_credentials(doc: &mut DocumentMut, credentials: &[GeminiCliCredential]) {
    ...
    doc["providers"]["geminicli"]["credentials"] = Item::ArrayOfTables(array);
}
