...
use crate::providers::geminicli::GeminiCliCredential;
use crate::providers::geminicli::constants::{
    GEMINICLI_AUTHORIZE_URL, GEMINICLI_CLIENT_ID, GEMINICLI_CLIENT_SECRET,
    GEMINICLI_REDIRECT_URI, GEMINICLI_SCOPE, GEMINICLI_TOKEN_URL, GEMINICLI_USER_AGENT,
};
...
pub(crate) async fn geminicli_oauth_start(
    Extension(ctx): Extension<Arc<AppContext>>,
    headers: HeaderMap,
    Query(query): Query<OAuthStartQuery>,
) -> Result<Response, StatusCode> {
    ensure_admin(&headers, &ctx)?;
    let auth_url = geminicli_oauth_authorize_url(query.project_id).await?;
...
pub(crate) async fn geminicli_oauth_authorize_url(
    project_id: Option<String>,
) -> Result<String, StatusCode> {
    let redirect_uri = GEMINICLI_REDIRECT_URI.to_string();
...
pub(crate) async fn geminicli_oauth_callback(
    Extension(ctx): Extension<Arc<AppContext>>,
    ...
    if project_id.is_none()
        && let Ok(setting) = ctx.geminicli().get_config().await
    ...
    let scope = parse_scope(tokens.scope.as_deref(), GEMINICLI_SCOPE);

    let credential = GeminiCliCredential {
        project_id: project_id.clone(),
        client_email: String::new(),
        client_id: GEMINICLI_CLIENT_ID.to_string(),
        client_secret: GEMINICLI_CLIENT_SECRET.to_string(),
        token: tokens.access_token,
        refresh_token: tokens.refresh_token.unwrap_or_default(),
        scope,
        token_uri: GEMINICLI_TOKEN_URL.to_string(),
        expiry,
        states: CredentialStatusList::default(),
    };

    let storage = ctx.geminicli();
    ...
    for spec in DEFAULT_USAGE_VIEWS {
        ...
        ctx.schedule_usage_anchor(ProviderKind::GeminiCli, spec.name, next_until)
            .await;
    }
...
async fn exchange_code_for_tokens(
    ...
    let body = url::form_urlencoded::Serializer::new(String::new())
        ...
        .append_pair("client_id", GEMINICLI_CLIENT_ID)
        .append_pair("client_secret", GEMINICLI_CLIENT_SECRET)
...
fn build_authorize_url(
    ...
    let mut url = Url::parse(GEMINICLI_AUTHORIZE_URL)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    {
        let mut pairs = url.query_pairs_mut();
        pairs.append_pair("client_id", GEMINICLI_CLIENT_ID);
        pairs.append_pair("redirect_uri", redirect_uri);
        pairs.append_pair("scope", GEMINICLI_SCOPE);
