...
impl ProviderAdmin for OpenAIAdmin {
    ...
    async fn add_credential(&self, ctx: &AppContext, credential: Value) -> Result<(), StatusCode> {
        ...
        let _ = set_default_usage_anchors(ctx.usage_store().as_ref(), ProviderKind::OpenAI, now)
            .await;
        for spec in DEFAULT_USAGE_VIEWS {
            ...
            ctx.schedule_usage_anchor(ProviderKind::OpenAI, spec.name, next_until)
                .await;
        }
        Ok(())
    }
