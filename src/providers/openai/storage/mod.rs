...
use crate::providers::openai::{OpenAICredential, OpenAISetting};

pub struct OpenAIStorage<'a, S> {
    storage: &'a S,
}
    ...
    S: OpenAIBackend,
{
    ...
    pub async fn get_config(&self) -> Result<OpenAISetting> {
        self.storage.get_config().await
    }

    pub async fn load_config(&self) -> Result<OpenAISetting> {
    ...
    pub async fn get_credentials(&self) -> Result<Vec<OpenAICredential>> {
        self.storage.get_credentials().await
    }

    pub async fn load_credentials(&self) -> Result<Vec<OpenAICredential>> {
        self.storage.load_credentials().await
    }

    pub async fn add_credential(&self, credential: OpenAICredential) -> Result<()> {
        self.storage.add_credential(credential).await
    }

    pub async fn get_credential(&self, index: usize) -> Result<Option<OpenAICredential>> {
...
crate::provider_storage_backend!(OpenAIBackend, OpenAISetting, OpenAICredential);
