...
pub mod config {
    ...
    #[sea_orm(table_name = "openai_config")]
    pub struct Model {
        #[sea_orm(primary_key)]
        pub id: i32,
        pub base_url: String,
...
pub mod credential {
    ...
    #[derive(Clone, Debug, PartialEq, Eq, DeriveEntityModel)]
    #[sea_orm(table_name = "openai_credentials")]
    pub struct Model {
        #[sea_orm(primary_key)]
        pub key: String,
        pub states: String,
...
pub async fn load_setting<C>(conn: &C) -> Result<OpenAISetting>
where
    C: ConnectionTrait,
{
    let model = config::Entity::find_by_id(1).one(conn).await?;
    if let Some(model) = model {
        return Ok(OpenAISetting {
            base_url: Url::parse(&model.base_url)?,
            rotate_num: model.rotate_num as u32,
        });
    }

    let setting = OpenAISetting::default();
    write_setting(conn, &setting).await?;
    Ok(setting)
}

pub async fn load_credentials<C>(conn: &C) -> Result<Vec<OpenAICredential>>
where
    C: ConnectionTrait,
{
...
pub async fn write_setting<C>(conn: &C, setting: &OpenAISetting) -> Result<()>
where
    C: ConnectionTrait,
{
    let model = config::Entity::find_by_id(1).one(conn).await?;
...
pub async fn write_credentials<C>(conn: &C, credentials: &[OpenAICredential]) -> Result<()>
where
    C: ConnectionTrait,
{
...
#[async_trait]
impl OpenAIBackend for DatabaseStorage {
