...
static ANTIGRAVITY_ROTATE_COUNTER: AtomicU64 = AtomicU64::new(0);

#[derive(Debug, Deserialize, Serialize, Clone, Default)]
pub struct AntigravityProvider {
    #[serde(default = "default_antigravity_setting")]
    pub setting: AntigravitySetting,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub credentials: Vec<AntigravityCredential>,
    #[serde(skip, default)]
...
impl AntigravityProvider {
    ...
    pub fn pick_credential(&self) -> Option<&AntigravityCredential> {
        self.pick_credential_for_model(DEFAULT_MODEL_KEY)
    }

    pub fn pick_credential_for_model(&self, model: &str) -> Option<&AntigravityCredential> {
        let now = now_timestamp();
        let valid: Vec<&AntigravityCredential> = self
            .credentials
            .iter()
            .filter(|item| {
        ...
        let count = ANTIGRAVITY_ROTATE_COUNTER.fetch_add(1, Ordering::Relaxed);
...
#[serde(default)]
pub struct AntigravitySetting {
    pub base_url: Url,
    #[serde(default = "default_rotate_num")]
    pub rotate_num: u32,
}

impl Default for AntigravitySetting {
    fn default() -> Self {
        Self {
            base_url: "https://daily-cloudcode-pa.sandbox.googleapis.com"
                .parse()
                .expect("valid antigravity base url"),
            rotate_num: default_rotate_num(),
        }
    }
}

fn default_antigravity_setting() -> AntigravitySetting {
    AntigravitySetting::default()
}
...
// storage integration lives in providers/antigravity/storage.rs
...
impl CredentialKey for AntigravityCredential {
    fn credential_key(&self) -> &str {
        self.project_id.as_str()
    }
}

impl Default for AntigravityCredential {
    fn default() -> Self {
        Self {
