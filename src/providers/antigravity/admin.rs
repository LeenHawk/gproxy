...
pub struct AntigravityAdmin;

#[async_trait]
impl ProviderAdmin for AntigravityAdmin {
    ...
    async fn add_credential(&self, ctx: &AppContext, credential: Value) -> Result<(), StatusCode> {
        ...
        if let Ok(usage) = fetch_antigravity_usage(ctx, &credential).await {
