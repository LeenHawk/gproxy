...
static VERTEXEXPRESS_ROTATE_COUNTER: AtomicU64 = AtomicU64::new(0);

#[derive(Debug, Deserialize, Serialize, Clone, Default)]
pub struct VertexExpressProvider {
    #[serde(default = "default_vertex_express_setting")]
    pub setting: VertexExpressSetting,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub credentials: Vec<VertexExpressCredential>,
    #[serde(skip, default)]
...
impl VertexExpressProvider {
    ...
    pub fn pick_credential(&self) -> Option<&VertexExpressCredential> {
        let now = now_timestamp();
        let valid: Vec<&VertexExpressCredential> = self
            .credentials
            .iter()
            .filter(|item| {
        ...
        let count = VERTEXEXPRESS_ROTATE_COUNTER.fetch_add(1, Ordering::Relaxed);
...
#[serde(default)]
pub struct VertexExpressSetting {
    pub base_url: Url,
    #[serde(default = "default_rotate_num")]
    pub rotate_num: u32,
}

impl Default for VertexExpressSetting {
    fn default() -> Self {
        Self {
            base_url: "https://aiplatform.googleapis.com"
                .parse()
                .expect("valid vertex express base url"),
...
impl CredentialKey for VertexExpressCredential {
    fn credential_key(&self) -> &str {
        self.key.as_str()
    }
}

fn default_vertex_express_setting() -> VertexExpressSetting {
    VertexExpressSetting::default()
}
...
// storage integration lives in providers/vertexexpress/storage.rs
