...
pub struct VertexExpressAdmin;

#[async_trait]
impl ProviderAdmin for VertexExpressAdmin {
    async fn get_config(&self, ctx: &AppContext) -> Result<Value, StatusCode> {
        let storage = ctx.vertexexpress();
        let config = storage
            .get_config()
            .await
    ...
    async fn add_credential(&self, ctx: &AppContext, credential: Value) -> Result<(), StatusCode> {
        let credential: VertexExpressCredential =
        ...
        for spec in DEFAULT_USAGE_VIEWS {
            ...
            ctx.schedule_usage_anchor(ProviderKind::VertexExpress, spec.name, next_until)
                .await;
        }
        Ok(())
    }
