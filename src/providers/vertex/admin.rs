...
pub struct VertexAdmin;

#[async_trait]
impl ProviderAdmin for VertexAdmin {
    async fn get_config(&self, ctx: &AppContext) -> Result<Value, StatusCode> {
        let storage = ctx.vertex();
        let config = storage
            .get_config()
            .await
    ...
    async fn add_credential(&self, ctx: &AppContext, credential: Value) -> Result<(), StatusCode> {
        let credential: VertexCredential =
        ...
        let _ = set_default_usage_anchors(ctx.usage_store().as_ref(), ProviderKind::Vertex, now)
            .await;
        for spec in DEFAULT_USAGE_VIEWS {
            ...
            ctx.schedule_usage_anchor(ProviderKind::Vertex, spec.name, next_until)
                .await;
        }
        Ok(())
    }
    ...
    async fn delete_credential(&self, ctx: &AppContext, index: usize) -> Result<(), StatusCode> {
        ...
        storage
            .delete_credential(&credential.project_id)
            .await
            .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
        Ok(())
    }
