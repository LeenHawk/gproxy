...
pub mod config {
    ...
    #[sea_orm(table_name = "claudecode_config")]
    pub struct Model {
        #[sea_orm(primary_key)]
        pub id: i32,
        pub base_url: String,
...
pub mod credential {
    ...
    #[derive(Clone, Debug, PartialEq, Eq, DeriveEntityModel)]
    #[sea_orm(table_name = "claudecode_credentials")]
    pub struct Model {
...
pub async fn load_setting<C>(conn: &C) -> Result<ClaudeCodeSetting>
where
    C: ConnectionTrait,
{
    let model = config::Entity::find_by_id(1).one(conn).await?;
    if let Some(model) = model {
        return Ok(ClaudeCodeSetting {
            base_url: Url::parse(&model.base_url)?,
            console_base_url: ClaudeCodeSetting::default().console_base_url,
            rotate_num: model.rotate_num as u32,
        });
    }

    let setting = ClaudeCodeSetting::default();
    write_setting(conn, &setting).await?;
    Ok(setting)
}

pub async fn load_credentials<C>(conn: &C) -> Result<Vec<ClaudeCodeCredential>>
where
    C: ConnectionTrait,
{
    ...
    let credentials = rows
        .into_iter()
        .map(|model| ClaudeCodeCredential {
...
pub async fn write_setting<C>(conn: &C, setting: &ClaudeCodeSetting) -> Result<()>
where
    C: ConnectionTrait,
{
    let model = config::Entity::find_by_id(1).one(conn).await?;
...
pub async fn write_credentials<C>(conn: &C, credentials: &[ClaudeCodeCredential]) -> Result<()>
where
    C: ConnectionTrait,
{
    ...
    for credential in credentials {
        let active = credential::ActiveModel {
            session_key: Set(credential.session_key.clone()),
...
#[async_trait]
impl ClaudeCodeBackend for DatabaseStorage {
