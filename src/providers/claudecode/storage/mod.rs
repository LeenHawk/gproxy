...
use crate::providers::claudecode::{ClaudeCodeCredential, ClaudeCodeSetting};

pub struct ClaudeCodeStorage<'a, S> {
    storage: &'a S,
}

#[async_trait]
pub trait ClaudeCodeBackend: Send + Sync {
...
impl<'a, S> ClaudeCodeStorage<'a, S>
where
    S: ClaudeCodeBackend,
{
    ...
    pub async fn get_config(&self) -> Result<ClaudeCodeSetting> {
        self.storage.get_config().await
    }

    pub async fn load_config(&self) -> Result<ClaudeCodeSetting> {
    ...
    pub async fn get_credentials(&self) -> Result<Vec<ClaudeCodeCredential>> {
        self.storage.get_credentials().await
    }

    pub async fn load_credentials(&self) -> Result<Vec<ClaudeCodeCredential>> {
        self.storage.load_credentials().await
    }

    pub async fn add_credential(&self, credential: ClaudeCodeCredential) -> Result<()> {
    ...
    pub async fn get_credential(&self, index: usize) -> Result<Option<ClaudeCodeCredential>> {
...
crate::provider_storage_backend!(ClaudeCodeBackend, ClaudeCodeSetting, ClaudeCodeCredential);
