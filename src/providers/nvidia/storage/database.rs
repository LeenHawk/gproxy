...
pub mod config {
    ...
    #[sea_orm(table_name = "nvidia_config")]
    pub struct Model {
        #[sea_orm(primary_key)]
        pub id: i32,
        pub base_url: String,
...
pub mod credential {
    ...
    #[derive(Clone, Debug, PartialEq, Eq, DeriveEntityModel)]
    #[sea_orm(table_name = "nvidia_credentials")]
    pub struct Model {
        #[sea_orm(primary_key)]
        pub key: String,
        pub states: String,
...
pub async fn load_setting<C>(conn: &C) -> Result<NvidiaSetting>
where
    C: ConnectionTrait,
{
    let model = config::Entity::find_by_id(1).one(conn).await?;
    if let Some(model) = model {
        return Ok(NvidiaSetting {
            base_url: Url::parse(&model.base_url)?,
            rotate_num: model.rotate_num as u32,
        });
    }

    let setting = NvidiaSetting::default();
    write_setting(conn, &setting).await?;
    Ok(setting)
}

pub async fn load_credentials<C>(conn: &C) -> Result<Vec<NvidiaCredential>>
where
    C: ConnectionTrait,
{
...
pub async fn write_setting<C>(conn: &C, setting: &NvidiaSetting) -> Result<()>
where
    C: ConnectionTrait,
{
    let model = config::Entity::find_by_id(1).one(conn).await?;
...
pub async fn write_credentials<C>(conn: &C, credentials: &[NvidiaCredential]) -> Result<()>
where
    C: ConnectionTrait,
{
...
#[async_trait]
impl NvidiaBackend for DatabaseStorage {
