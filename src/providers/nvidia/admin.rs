...
pub struct NvidiaAdmin;

#[async_trait]
impl ProviderAdmin for NvidiaAdmin {
    async fn get_config(&self, ctx: &AppContext) -> Result<Value, StatusCode> {
        let storage = ctx.nvidia();
        let config = storage
            .get_config()
            .await
    ...
    async fn add_credential(&self, ctx: &AppContext, credential: Value) -> Result<(), StatusCode> {
        let credential: NvidiaCredential =
        ...
        let _ = set_default_usage_anchors(ctx.usage_store().as_ref(), ProviderKind::Nvidia, now)
            .await;
        for spec in DEFAULT_USAGE_VIEWS {
            ...
            ctx.schedule_usage_anchor(ProviderKind::Nvidia, spec.name, next_until)
                .await;
        }
        Ok(())
    }
