...
- **Multi-provider service-tier billing.** Price rules can apply model-specific multipliers or category rates to `service_tier`, Gemini `serviceTier`, and Claude `speed`. Admission uses the requested tier, while final settlement prefers the tier actually reported by OpenAI-compatible responses, Claude usage, Gemini usage/headers, and Bedrock conversions, so graceful downgrades use standard pricing. The bundled catalog includes confirmed OpenAI Fast/Flex, Gemini Priority/Flex, Claude Opus Fast, and xAI Priority rates; custom Groq, Bedrock, and enterprise tiers use the same format.
...
- **Antigravity request compatibility.** OpenAI's `store` option is removed before Gemini requests are wrapped in the Code Assist envelope, avoiding an upstream unknown-field rejection.
...
- **多供应商服务层级计费。** 价格规则现在可为 `service_tier`、Gemini `serviceTier` 或 Claude `speed` 配置模型级倍率或各 token 类别价格。额度预估使用请求层级，最终结算则优先采用 OpenAI 兼容响应、Claude usage、Gemini usage/响应头及 Bedrock 转换实际报告的层级，因此上游降级会按标准价结算。内置价格表已加入经官方确认的 OpenAI Fast/Flex、Gemini Priority/Flex、Claude Opus Fast 与 xAI Priority 价格；Groq、Bedrock 和企业合同层级也使用同一配置格式。
...
- **Antigravity 请求兼容性。** Gemini 请求封装到 Code Assist envelope 前会移除 OpenAI `store` 选项，避免上游返回未知字段错误。
