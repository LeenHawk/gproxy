...
- **Three runtime form factors from one engine.** The same crate compiles to a native binary, a distroless Docker image, and a `wasm32` edge library. Edge wasm is live-verified on Cloudflare Workers, Netlify, and Deno Deploy; prebuilt one-click bundles ship on the `deploy` branch.
...
- **同源三形态.** 同一个 crate 编译出原生二进制、distroless Docker 镜像和 `wasm32` 边缘库。边缘 wasm 已在 Cloudflare Workers、Netlify、Deno Deploy 实测跑通;预构建一键产物放在 `deploy` 分支。
