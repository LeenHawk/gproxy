...
| 凭证策略 | `round_robin`（默认）、`sticky` 或 `earliest_reset`，见下文。 |
...
| `earliest_reset` | 优先使用当前模型下仍有额度、且下次重置时间最近的凭据；时间相同时按权重轮询。 |

### 额度即将重置优先

供应商凭据策略可选 `earliest_reset`（控制台：**额度即将重置优先**）。
默认策略不变，无需数据库迁移。它仅比较当前请求解析后的上游模型所适用的
有效额度窗口，优先使用下次重置时间最近的账号；例如同时考虑 5 小时和周额，
但不会混用 Gemini 与 Claude 的额度。90%～99% 已用仍视为有剩余额度。

任一适用窗口已耗尽时，该账号排到后面作为回退；异常账号及渠道故障转移层级
仍遵循现有规则。重置时间相同则在同组内按权重轮询。未知、过期的重置时间
不会抢在已知可用窗口前面；没有可用日期时退回加权轮询。

调度读取已缓存的上游额度信息，更新额度后自动重新选择，不会在每个请求前
同步查询额度，也不会把 OAuth 令牌续期时间当成额度重置时间。
