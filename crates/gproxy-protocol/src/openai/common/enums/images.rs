...
strict_string_enum!(ImageModeration { Low => "low", Auto => "auto" });
strict_string_enum!(ImageOutputFormat { Png => "png", Jpeg => "jpeg", Webp => "webp" });
...
strict_string_enum!(ImageResponseQuality { Low => "low", Medium => "medium", High => "high" });
strict_string_enum!(ImageResponseFormat { Url => "url", B64Json => "b64_json" });
...
strict_string_enum!(ImageStyle { Vivid => "vivid", Natural => "natural" });
strict_string_enum!(ImageInputFidelity { High => "high", Low => "low" });
