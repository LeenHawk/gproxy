...
pub fn transform_request(request: GeminiCountTokensRequest) -> OpenAIInputTokenCountRequest {
    ...
    let mut contents = request.body.contents.unwrap_or_default();
    let mut tools = None;
    ...
    let mut openai_tools = tools.map(map_tools).unwrap_or_default();
    if let Some(image_tool) = map_image_generation_tool(generation_config.as_ref()) {
    ...
    let text = map_response_format(generation_config.as_ref());
    let reasoning = map_reasoning(generation_config.as_ref());
...
fn map_parts_to_input_contents(parts: &[GeminiPart]) -> Vec<InputContent> {
    ...
    for part in parts {
        ...
        // Gemini tool parts have no OpenAI input equivalent; serialize to text to preserve content.
...
fn push_inline_blob(contents: &mut Vec<InputContent>, blob: &GeminiBlob) {
    // OpenAI input images cannot carry inline base64 data, so store as a file payload instead.
...
fn map_tools(tools: Vec<GeminiTool>) -> Vec<Tool> {
    ...
    for tool in tools {
        ...
        if tool.computer_use.is_some() {
            // Gemini doesn't provide display size; fall back to a reasonable browser default.
