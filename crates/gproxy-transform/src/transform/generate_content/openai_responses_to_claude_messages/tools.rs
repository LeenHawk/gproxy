...
pub(super) fn response_tools_to_claude(tools: Option<Vec<openai::ResponseTool>>) -> ClaudeTools {
    ...
    let programmatic = web_search || output.iter().any(tool_activates_programmatic_calling);
    ClaudeTools {
...
mod tests {
    ...
    #[test]
    fn preserves_responses_mcp_and_code_execution() {
        let mapped = response_tools_to_claude(Some(vec![
            openai::ResponseTool::Mcp {
                server_label: "docs".to_owned(),
                allowed_tools: Some(openai::McpAllowedTools::Names(vec!["search".to_owned()])),
        ...
        let servers = mapped.mcp_servers.unwrap();
        assert_eq!(servers[0].name, "docs");
        assert_eq!(servers[0].url, "https://mcp.example.test");
        assert_eq!(
            servers[0]
                .tool_configuration
                .as_ref()
        ...
        assert!(matches!(
            mapped.tools.unwrap()[0],
            claude::Tool::Command(claude::CommandTool::CodeExecution20260120(_))
        ));
        assert!(mapped.programmatic);
    }
}
