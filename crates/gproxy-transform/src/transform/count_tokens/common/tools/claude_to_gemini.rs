...
pub(in crate::transform::count_tokens) fn claude_tools_to_gemini(
    ...
    for tool in tools.into_iter().flatten() {
        match tool {
            ...
            claude::Tool::WebSearch(_) => gemini_tools.push(crate::protocol::wire!(gemini::Tool {
                google_search: Some(gemini::GoogleSearch::default()),
                ..Default::default()
            })),
            claude::Tool::WebFetch(_) => gemini_tools.push(crate::protocol::wire!(gemini::Tool {
                url_context: Some(gemini::UrlContext::default()),
                ..Default::default()
            })),
            claude::Tool::Computer(_) => gemini_tools.push(crate::protocol::wire!(gemini::Tool {
            ...
            claude::Tool::McpToolset(toolset) => {
                gemini_tools.push(crate::protocol::wire!(gemini::Tool {
    ...
    let mcp_servers = mcp_servers
        ...
        .flatten()
        .map(|server| {
            crate::protocol::wire!(gemini::McpServer {
                name: Some(server.name),
                streamable_http_transport: Some(crate::protocol::wire!(
