...
pub fn preserve_claude_input_transformations(
    extra: &mut claude::JsonObject,
    ...
    if let Some(transformations) = transformations {
        extra.insert(
            ...
            serde_json::to_value(transformations)
                .expect("Claude input transformations must serialize"),
        );
    }
}

pub fn apply_claude_message_controls(
            ...
            })
    {
        output_config
            .get_or_insert_with(|| {
                crate::protocol::wire!(claude::OutputConfig {
                    effort: None,
                    format: None,
                    task_budget: None,
...
mod tests {
    ...
    fn renders_turn_scoped_messages_and_applies_effort_for_current_user() {
        ...
        apply_claude_message_controls(&mut request.messages, &mut request.output_config);
