...
impl Store {
        ...
        self.backend()
            ...
            .map(|row| {
                Ok(crate::records::TokenizerVocabRecord {
                    ...
                    repository: row.text("repository")?.to_owned(),
    ...
    pub async fn tokenizer_vocab(
        &self,
        name: &str,
    ) -> Result<Option<crate::records::TokenizerVocabData>, StoreError> {
        let mut result = self.backend().execute(tokenizer::get(name)?).await?;
        let Some(row) = result.rows.pop() else {
            return Ok(None);
        };
        Ok(Some(crate::records::TokenizerVocabData {
            repository: row.text("repository")?.to_owned(),
            bytes: row.blob("bytes")?.to_vec(),
        }))
    }

    pub async fn put_tokenizer_vocab(
        &self,
        name: &str,
        repository: &str,
        bytes: &[u8],
        ...
        self.backend()
            .execute(tokenizer::put(name, repository, bytes, unix_now())?)
            .await?;
        Ok(())
    }
