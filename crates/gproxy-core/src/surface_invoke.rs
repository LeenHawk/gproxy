...
impl<H: Host> SurfaceInvoke for SurfaceCaller<'_, H> {
    fn invoke<'a>(
        ...
        Box::pin(async move {
            ...
            let key = request.key;
            let ctx = crate::surface_reply::request_ctx(&self.target, &request, request_id.clone());
            let Some(target) = self.target(request.credential) else {
                ...
                return Ok(crate::surface_reply::error(error));
            };
    ...
    fn fetch_presigned<'a>(
        ...
        Box::pin(async move {
            ...
            crate::surface_reply::from_outcome(funnel::free_streaming(
