...
use super::{random_project_id, ANTIGRAVITY_USER_AGENT, DEFAULT_BASE_URL, PROVIDER_NAME};
...
pub(super) async fn handle_oauth_start(
    ...
    let meta = UpstreamRecordMeta {
        ...
        credential_id: None,
        operation: "antigravity.oauth".to_string(),
        model: None,
        request_method: "GET".to_string(),
        request_path: "/antigravity/oauth".to_string(),
        request_query: query,
...
pub(super) async fn handle_oauth_callback(
    ...
    let meta = UpstreamRecordMeta {
        ...
        provider_id: ctx.provider_id,
        credential_id: None,
        operation: "antigravity.oauth.callback".to_string(),
        model: None,
        request_method: "GET".to_string(),
        request_path: "/antigravity/oauth/callback".to_string(),
        request_query: query,
        request_headers: String::new(),
        request_body: String::new(),
    };
...
pub(super) async fn detect_project_id(
    access_token: &str,
    base_url: &str,
    user_agent: &str,
    proxy: Option<&str>,
...
async fn persist_credential(
    ...
    let credential_name = email
        .as_ref()
        .map(|value| format!("antigravity:{value}"))
        .or_else(|| Some(format!("antigravity:{project_id}")));
