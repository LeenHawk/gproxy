...
for (const entry of await readdir(embedDir)) {
  if (entry !== ".gitkeep") await rm(path.join(embedDir, entry), { recursive: true, force: true })
}
for (const entry of await readdir(distDir)) {
  await cp(path.join(distDir, entry), path.join(embedDir, entry), { recursive: true, force: true })
}
