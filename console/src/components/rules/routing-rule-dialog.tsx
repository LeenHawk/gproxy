...
export function RoutingRuleDialog({ providerId, channel, rule, defaults, trigger, saving, onSave }: {
  providerId: number
  channel?: ChannelDto
  rule?: RoutingRuleDto
  ...
  const operations = useMemo(() => [...new Set(supports.map((support) => support.operation))], [supports])
  const kinds = useMemo(() => [...new Set(supports.filter((support) => support.operation === operation).map((support) => support.source))], [operation, supports])
  const targets = useMemo(() => supports.filter((support) => support.operation === operation && support.source === kind), [kind, operation, supports])
  const reset = () => {
  ...
  return <Dialog open={open} onOpenChange={(next) => { if (next) reset(); setOpen(next) }}><DialogTrigger asChild>{trigger}</DialogTrigger><DialogContent showCloseButton={false}><form className="flex min-h-0 flex-1 flex-col" onSubmit={(event) => void submit(event)}><DialogHeader><DialogTitle>{t(rule ? "rules.routing.edit" : "rules.routing.add")}</DialogTitle></DialogHeader><DialogBody><FieldGroup>
    ...
    <Field><FieldLabel htmlFor="routing-implementation">{t("rules.fields.implementation")}</FieldLabel><Select name="routing-implementation" value={implementation} onValueChange={(value) => setImplementation(value as RoutingImplementationDto)}><SelectTrigger id="routing-implementation" className="w-full"><SelectValue /></SelectTrigger><SelectContent>{IMPLEMENTATIONS.map((value) => <SelectItem key={value} value={value}>{t(`rules.values.${value}`)}</SelectItem>)}</SelectContent></Select></Field>
    {implementation === "transform_to" ? <><Field><FieldLabel htmlFor="routing-dest-operation">{t("rules.fields.destinationOperation")}</FieldLabel><SearchableSelect {...selectProps} id="routing-dest-operation" ariaLabel={t("rules.fields.destinationOperation")} value={destOperation} options={[...new Set(targets.map((target) => target.target_operation))].map((value) => ({ value, label: t(`rules.operations.${value}`, { defaultValue: value }) }))} onChange={setDestOperation} /></Field><Field><FieldLabel htmlFor="routing-dest-kind">{t("rules.fields.destinationKind")}</FieldLabel><SearchableSelect {...selectProps} id="routing-dest-kind" ariaLabel={t("rules.fields.destinationKind")} value={destKind} options={[...new Set(targets.map((target) => target.target))].map((value) => ({ value, label: t(`rules.wires.${value}`, { defaultValue: value }) }))} onChange={setDestKind} /></Field></> : null}
