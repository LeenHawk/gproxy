...
export function IdentityTable(props: IdentityTableProps) {
  ...
  const organizationActions = (value: OrganizationDto) => <span className="flex items-center justify-end gap-2" onClick={(event) => event.stopPropagation()}><Toggle name={value.name} enabled={value.enabled} pending={props.pending} onChange={() => props.onOrganizationToggle(value)} /><EntityDeleteButton entity="organizations" id={value.id} label={value.name} queryKeys={["organizations", "teams", "users"]} /></span>
