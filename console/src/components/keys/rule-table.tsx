...
export function RuleTable(props: RuleTableProps) {
  ...
  return (
    <DataTable columns={columns} rows={props.rows} rowKey={(row) => row.id} searchText={(row) => `${row.subject} ${row.detail}`} renderCard={(row) => <div className="flex items-start justify-between gap-3"><div><p className="font-mono text-xs">{row.subject}</p><p className="mt-1 font-mono text-xs text-muted-foreground">{row.detail}</p></div>{remove(row)}</div>} empty={props.empty} storageKey={props.storageKey ?? "access-rules"} selectable />
  )
}
