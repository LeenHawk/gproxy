...
export function ChannelCatalog({ channels }: { channels: Array<ChannelDto> }) {
  ...
  const columns: Array<DataTableColumn<ChannelRow>> = [
    ...
    { key: "operation", label: t("channels.operation"), header: t("channels.operation"), cell: ({ support }) => <span className="font-mono text-xs">{support?.operation ?? "—"}</span> },
    { key: "source", label: t("channels.source"), header: t("channels.source"), cell: ({ support }) => <span className="font-mono text-xs">{support?.source ?? "—"}</span> },
    { key: "target", label: t("channels.target"), header: t("channels.target"), cell: ({ support }) => <span className="inline-flex items-center gap-1.5 font-mono text-xs">{support ? <ArrowRightIcon className="text-muted-foreground" aria-hidden /> : null}{support?.target ?? "—"}</span> },
  ]
  return (
