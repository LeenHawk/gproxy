...
import { CircleDollarSignIcon, PencilIcon, PlusIcon, Trash2Icon } from "lucide-react"
...
export function PricingWorkspace({ rules, rates, providers }: { rules: Array<PriceRuleDto>; rates: Array<PriceRateDto>; providers: Array<ProviderDto> }) {
  ...
  const ruleColumns: Array<DataTableColumn<PriceRuleDto>> = [
    { key: "model", label: t("pricing.rules.pattern"), header: t("pricing.rules.pattern"), cell: (rule) => <span className="flex items-center gap-2 font-mono text-xs"><CircleDollarSignIcon aria-hidden />{rule.model_pattern}</span> },
    ...
    { key: "actions", label: t("common.actions.edit"), header: <span className="sr-only">{t("common.actions.edit")}</span>, cell: (rule) => <div onClick={(event) => event.stopPropagation()}><PriceRuleDialog rule={rule} providers={providers} trigger={<Button size="icon-sm" variant="outline" aria-label={t("common.actions.edit")}><PencilIcon aria-hidden /></Button>} /></div> },
  ]
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid min-w-0 gap-5 md:grid-cols-[minmax(18rem,0.8fr)_minmax(0,1.2fr)]">
        <div className={cn(selected && "hidden md:block")}><DataTable columns={ruleColumns} rows={rules} rowKey={(rule) => rule.id} searchText={(rule) => `${rule.model_pattern} ${rule.provider_id == null ? t("pricing.rules.allProviders") : providerNames.get(rule.provider_id) ?? rule.provider_id}`} renderCard={(rule) => <div className="flex items-start justify-between gap-3"><div><p className="font-mono text-xs">{rule.model_pattern}</p><p className="text-xs text-muted-foreground">{rule.provider_id == null ? t("pricing.rules.allProviders") : providerNames.get(rule.provider_id) ?? rule.provider_id}</p></div><Badge variant={rule.enabled ? "outline" : "secondary"}>{t(`common.status.${rule.enabled ? "enabled" : "disabled"}`)}</Badge></div>} empty={t("pricing.rules.empty")} storageKey="price-rules" activeRowKey={selected?.id} selectable onRowClick={(rule) => navigateAdminPath(`/admin/pricing/${rule.id}/rates`)} /></div>
        <div className={cn("min-w-0", !selected && "hidden md:block")}>
          {selected ? <><Button className="mb-3 md:hidden" variant="ghost" onClick={() => navigateAdminPath(adminPath("pricing"))}>{t("common.actions.back")}</Button><Card><CardHeader><CardTitle className="font-mono">{selected.model_pattern}</CardTitle><CardAction><PriceRateDialog rules={rules} initialRuleId={selected.id} trigger={<Button size="sm" variant="outline"><PlusIcon data-icon="inline-start" />{t("pricing.rates.add")}</Button>} /></CardAction></CardHeader><CardContent><DataTable columns={rateColumns} rows={selectedRates} rowKey={(rate) => rate.id} searchText={(rate) => `${rate.metric} ${rate.price}`} renderCard={(rate) => <div className="flex flex-col gap-3"><div><p className="font-mono text-xs">{rate.metric}</p><p className="text-xs text-muted-foreground">{t("pricing.rates.summary", { price: rate.price, units: rate.unit_size })}</p></div>{rateActions(rate)}</div>} empty={t("pricing.rates.empty")} storageKey="price-rates" selectable /></CardContent></Card></> : <div className="grid min-h-80 place-items-center text-sm text-muted-foreground">{t("pricing.rules.selectPrompt")}</div>}
        </div>
      </div>
    </div>
  )
