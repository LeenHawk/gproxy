...
export function TeamDetail({ team, organizations, access, panel, onPanel, onDeleted }: {
  team: TeamDto
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <Tabs value={panel} onValueChange={(next) => onPanel(next as TeamPanel)}>
        ...
        <TabsContent value="profile" className="pt-5"><Card><CardHeader><CardTitle>{t("users.detail.profile")}</CardTitle></CardHeader><CardContent><dl className="grid gap-5 sm:grid-cols-2"><div><dt className="text-xs text-muted-foreground">{t("users.fields.organization")}</dt><dd className="mt-1 text-sm">{organization?.name ?? t("common.none")}</dd></div><div><dt className="text-xs text-muted-foreground">{t("common.status.label")}</dt><dd className="mt-1 text-sm">{t(`common.status.${team.enabled ? "enabled" : "disabled"}`)}</dd></div></dl></CardContent></Card></TabsContent>
