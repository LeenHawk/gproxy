  ...
  return (
    <Card size="sm" className="min-w-0">
      ...
      <CardContent className="flex flex-col gap-2">
        <DataTable columns={columns} rows={page.items} rowKey={(item) => item.id} searchText={(item) => `${item.method} ${item.path} ${item.request_id} ${item.response_status ?? ""} ${metrics(item)}`} renderCard={(item) => <div className="grid gap-1"><span className="flex items-center justify-between gap-3"><span className="truncate font-medium">{item.method} {item.path}</span><span className={cn("font-mono text-xs", item.response_status != null && item.response_status >= 400 ? "text-destructive" : "text-muted-foreground")}>{item.response_status ?? t("logs.pending")}</span></span><span className="truncate font-mono text-xs text-muted-foreground">{item.request_id}</span><span className="text-xs text-muted-foreground">{metrics(item)}</span><span className="text-xs text-muted-foreground">{formatInstant(item.at, i18n.language)}</span></div>} empty={t("logs.list.empty")} storageKey="logs" activeRowKey={page.items.find((item) => item.request_id === selected)?.id} onRowClick={(item) => onSelect(item.request_id)} />
