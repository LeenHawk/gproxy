...
export function UpdatePanel() {
  ...
  const apply = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      if (result.restart === "none") await queryClient.invalidateQueries({ queryKey: ["native-update"] })
    },
  ...
  return (
    <>
        ...
        {data ? (
          <CardFooter className="flex flex-wrap justify-end gap-2">
            ...
            <Button disabled={!data.available || busy} onClick={() => setConfirm("apply")}>
              {t(data.available ? "update.actions.apply" : "update.actions.current")}
            </Button>
          </CardFooter>
        ) : null}
      </Card>
      ...
      {data?.notes ? (
        <Card>
          <CardHeader><CardTitle>{t("update.notes.title")}</CardTitle></CardHeader>
          <CardContent><pre className="max-h-96 overflow-auto whitespace-pre-wrap font-sans text-sm leading-relaxed">{data.notes}</pre></CardContent>
        </Card>
      ) : null}
      ...
      <ConfirmDangerous
        ...
        title={t("update.actions.applyConfirmTitle")}
        description={t("update.actions.applyConfirmDescription")}
        confirmLabel={t("update.actions.apply")}
