...
export function OverviewPage() {
  ...
  const now = useNow()
  const [credentialQuery, usageQuery, quotaQuery, cycleQuery] = useQueries({ queries: [
  ...
  const queries = [credentialQuery, usageQuery, quotaQuery, cycleQuery]
  ...
  return (
    <PageLayout title={t("nav.overview")} description={t("usage.overviewDescription")}>
      <QueryState loading={queries.some((query) => query.isLoading)} error={error ? t("common.loadError") : ""}>
        <OverviewDashboard credentials={credentialQuery.data ?? []} usage={usageQuery.data ?? []} quotas={quotaQuery.data ?? []} cycles={cycleQuery.data ?? []} />
      </QueryState>
    </PageLayout>
  )
}
