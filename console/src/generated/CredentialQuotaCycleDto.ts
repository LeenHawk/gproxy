// This file was generated by [ts-rs](https://github.com/Aleph-Alpha/ts-rs). Do not edit this file manually.
import type { BoundaryConfidenceDto } from "./BoundaryConfidenceDto";
import type { BoundarySourceDto } from "./BoundarySourceDto";
import type { QuotaCoverageDto } from "./QuotaCoverageDto";
import type { QuotaCycleStatusDto } from "./QuotaCycleStatusDto";

export type CredentialQuotaCycleDto = { id: number, version: number, credential_id: number, window_key: string, period_start: number | null, period_end: number | null, boundary_source: BoundarySourceDto, boundary_confidence: BoundaryConfidenceDto, status: QuotaCycleStatusDto, close_reason: string | null, last_observed_at: number, upstream_used: string | null, upstream_limit: string | null, used_percent: string | null, coverage: QuotaCoverageDto, metrics: unknown, };
