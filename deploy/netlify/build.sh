...
PUBLIC="$CRATE_ROOT/deploy/netlify/public"
...
find "$PUBLIC" -mindepth 1 -maxdepth 1 ! -name "_redirects" ! -name ".gitkeep" ! -name "console" -exec rm -rf {} +
...
if [ -f "$CONSOLE_ASSETS/index.html" ]; then
  ...
  echo "warning: $CONSOLE_ASSETS/index.html not found; Netlify console assets not bundled" >&2
fi
echo "done: $OUT"
